...
Plantilla específica para issues de tipo bug. Usar al crear cualquier issue con `type/bug`. Es más estricta que la de feature porque sin repro no hay bug accionable.
...
1. Ir a https://example.com/registro
...
→ Buen candidato para `actor/ai`. Repro claro, test definido, archivo identificado.

### Ejemplo 2 — Bug intermitente (NO delegable inicialmente)

```markdown
# Title
Sesiones se cierran aleatoriamente en la plataforma de streaming

## Síntoma

Usuarios reportan que estando viendo un video la sesión se cierra
...
Sesión persiste mientras se ve contenido (token de 24h).
...
→ NO delegar a IA. Necesita debugging exploratorio. Asignar a humano. Primer paso: **añadir más logging** (sub-issue `type/chore`).
