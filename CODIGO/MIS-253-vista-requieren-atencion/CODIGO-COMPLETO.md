# MIS-253 — Vista: Leads sin próximo paso («Requieren atención») — código completo

Ver plan y evidencia de verificación en [`PLANS/MIS-253-vista-requieren-atencion.md`](../../PLANS/MIS-253-vista-requieren-atencion.md).

3 archivos tocados: `convex/contacts.ts` (editar), `src/app/(app)/(with-nav)/pendientes/page.tsx` (editar), `e2e/full-flow.spec.ts` (editar, fix permanente de escopado — ver hallazgo 3 del plan).

---

## `convex/contacts.ts` (EDITAR — archivo completo)
...
// programados, ver listDueToday en convex/reminders.ts). Vive en
// contacts.ts, no en reminders.ts: el resultado tiene forma de "contacto"
// (name/status/lastContactAt), mismo criterio que getPipelineSummary
// (agregación sobre contacts aunque cruce con otras tablas).
...
## `src/app/(app)/(with-nav)/pendientes/page.tsx` (EDITAR — archivo completo)
...
## `e2e/full-flow.spec.ts` (EDITAR — solo pasos 7 y 8 cambian, resto igual)

Fix permanente de escopado (hallazgo 3 de la verificación, ver plan): sin esto, el test rompía en el paso 8 porque el contacto, al completarse su recordatorio, pasa a aparecer (correctamente) en "Requieren atención", y el selector de `<li>` sin escopar encontraba esa fila nueva en vez de comprobar que la de "Para hoy" había desaparecido.

```tsx
  await test.step("7. Aparece en Pendientes del día", async () => {
    await page.goto("/pendientes");
    // Escopado a la sección "Para hoy" (MIS-253 añadió una segunda sección,
    // "Requieren atención", debajo — un <li> sin escopar podría matchear la
    // fila equivocada si el mismo contacto apareciera en ambas).
    const todaySection = page.locator("section").filter({ has: page.getByRole("heading", { name: "Para hoy" }) });
    const row = todaySection.getByRole("listitem").filter({ hasText: contactName });
  ...
  await test.step("8. Marca el seguimiento como hecho desde Pendientes", async () => {
    // Mismo escopado que el paso 7. Real e intencional (MIS-253): al
    // completarse, este contacto (estado activo, ya sin seguimiento
    // pendiente) pasa a aparecer en "Requieren atención" — un <li> sin
    // escopar seguiría viendo su nombre visible ahí y el toBeHidden() de
    // abajo fallaría, aunque la fila de "Para hoy" sí haya desaparecido.
    const todaySection = page.locator("section").filter({ has: page.getByRole("heading", { name: "Para hoy" }) });
    const row = todaySection.getByRole("listitem").filter({ hasText: contactName });
