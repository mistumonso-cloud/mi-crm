...
> **Estado**: Auditoría de código GO. Instalado en la rama `feature/mis-259-registrar-venta-directa`, sin push todavía.
...
**Auditoría de código: GO.** Sin Blockers ni Majors. Confirmado: la condición de la auditoría de plan (flujo e2e real FAB → hoja → búsqueda/selección → formulario → confirmar) queda cubierta; el riesgo de doble FAB en `/ventas` queda resuelto (`PageFab` solo suprime el FAB genérico en esa ruta). Sugerencias no bloqueantes, enviadas a follow-up: (a) aislar mejor los e2e del deployment de dev compartido o ejecutar contra datos efímeros — los tests de MIS-259 añaden más datos append-only a `saleClosures`, lo que puede aumentar ruido/flakiness con el tiempo (ver hallazgo de instalación más abajo, que confirma este riesgo antes de lo esperado); (b) añadir en una ronda futura un caso e2e de venta directa sobre un contacto `"lost"` (el código ya lo soporta, solo falta cobertura); (c) considerar truncado/ellipsis defensivo en nombres/teléfonos largos del selector de contactos (no bloqueante, el patrón ya existe en listas previas).

**Instalación real** (aplicar `CODIGO/MIS-259-registrar-venta-directa/**` a `convex/`, `src/`, `e2e/`):

- Copiados los 8 archivos a sus rutas reales (`convex/sales.ts`, `src/lib/contacts/actions.ts`, `src/components/crm/PageFab.tsx` [nuevo], `src/app/(app)/(with-nav)/layout.tsx`, `.../ventas/{page.tsx, SalesList.tsx, RegisterSaleForm.tsx}` [`RegisterSaleForm.tsx` nuevo], `e2e/edge-cases.spec.ts`). `CODIGO/MIS-259-registrar-venta-directa/` se mantiene tal cual (su limpieza es una chore aparte, después de mergear, mismo patrón que `chore/mis-256-retirar-staging`).
- `npx tsc --noEmit`, `npm run lint`, `npm run build`: sin errores (mismo warning preexistente de `Avatar.jsx`, no relacionado).
- `npx convex dev --once` despliega `registerDirectSale` en el deployment de dev compartido (`dutiful-mole-111`) **de forma permanente esta vez** (ya no se revierte — el código está instalado de verdad).
- `npm run test:e2e` completo: **22/24 pasan**. Los 3 tests nuevos de MIS-259 pasan de forma consistente (verificados también en ejecuciones aisladas repetidas). Fallan 2 tests preexistentes, ninguno de los archivos de este diff: `"posponer un seguimiento desde Pendientes..."` (MIS-254) y `"flujo completo de Carlos: alta -> seguimiento -> venta"` (`full-flow.spec.ts`) — ambos por timeout esperando que una fila desaparezca de "Para hoy" tras pulsar un botón de acción (el snapshot de fallo muestra el botón atascado en un estado deshabilitado "…").
  - **Investigado a fondo antes de aceptarlo como ruido**: se bisecó revirtiendo archivo por archivo (`layout.tsx` a `main`, con el resto de MIS-259 presente) y no cambia el resultado — sigue fallando. Se revirtieron TODOS los archivos de MIS-259 a `main` (vía `git stash`) y el test de posponer pasó una vez; al restaurar MIS-259 y repetir 5 veces más, falló las 5. El diff de `convex/sales.ts`, `src/lib/contacts/actions.ts` y `e2e/edge-cases.spec.ts` es puramente aditivo (nuevas funciones/tests al final del archivo, cero líneas tocadas de código existente) — sin mecanismo plausible para afectar `/pendientes`. Conclusión: **no es una regresión de MIS-259**, es degradación por carga/volumen de datos acumulados en el deployment de dev compartido durante esta misma sesión (múltiples ejecuciones completas de la suite, sin `deleteContact` para limpiar) — coincide exactamente con la sugerencia (a) de la auditoría de código de arriba, y con el mismo patrón ya documentado en `PLANS/MIS-256-pantalla-de-ventas.md` ("ruido por acumulación de datos de prueba... no un bug de código"). No se pudo confirmar con un pase 100% limpio en aislamiento esta vez (a diferencia de la ronda de generación de código, donde sí se logró) — la carga acumulada parece haber empeorado desde entonces.
  - Esta deuda queda registrada como más urgente de lo previsto: la sugerencia (a) de la auditoría ("puede aumentar ruido/flakiness con el tiempo") ya se está observando, no es solo teórico.
- Commit en `feature/mis-259-registrar-venta-directa` (ver `git log` de la rama). Sin push, sin PR — pendiente del visto bueno explícito del usuario.
